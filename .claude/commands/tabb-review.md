...
Evaluate against the following criteria, in priority order:

## Bugs

- Logic errors, race conditions, and data inconsistency
- Unhandled promise rejections or errors swallowed silently at the wrong level
- Wrong values passed to functions; incorrect assumptions about external API responses
- Off-by-one errors, incorrect expiry/retry behavior
- Security: unvalidated input at system boundaries, injection risk
- Log messages that misrepresent what actually happened

## Code Efficiency

- N+1 database queries; redundant DB round-trips that could be batched
- Sequential `await`s that should be `Promise.all`
- Heavy objects (clients, factories, repos) instantiated per-request when they could be shared
- Large transactions likely to cause lock contention
...
- React hooks follow the rules of hooks; dependency arrays are correct
